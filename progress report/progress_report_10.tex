...
\begin{document}
	...
	The Hadamard can be used to put any qubit in a superposition. If it takes $\ket{0}$, then the output will be 
	...
	Furthermore, the Quantum Fourier Transform (QFT) is one of the building blocks in quantum computers and many advanced algorithms are based on it, but it needs quantum computers that are immune to noise. QFT is represented for any basis state $J$ as:
	...
	For the optimization problem of a hard margin, for a dataset that is perfectly linearly separable, we can find the optimal hyperplane by solving a constrained optimization problem.
 ...
 For mutation analysis, mutations are created from the original program by presenting errors, based on mutation operators, in the original source code. Below is Table \ref{tab:mutation_analysis} showing the possible results for each mutant after testing:
	...
	Statistical testing is used to determine if there are any statistically significant differences between the data used in testing or due to a random chance, based on p-value. The test is structured around two competing claims regarding a population parameter (e.g., the mean, $\mu$):
	...
	\subsection{Metamorphic Testing}
		\label{sec:LiteratureReview-Metamorphic-Testing}
	...
	One of the predominant software testing methods is Metamorphic Testing (MT). Metamorphic testing (MT) is a property-based testing method introduced by Chen \cite{mt1998}. Metamorphic testing has gained popularity in testing complex systems, e.g., machine learning. In essence, MT is a convenient method to test complicated systems as black boxes. MT relies on metamorphic relations (MRs). These MRs are created to relate two inputs in a way such that we can reason about the relation between their corresponding outputs \cite{xu2024mrscoutautomatedsynthesismetamorphic}. If the relation between two outputs does not hold then we can conclude that there is a bug in the SUT.
	
	A practical example is the work of Mao et al. \cite{cmao2024}, where the authors assessed the use of metamorphic relations in recommender systems by empirical evaluation on open-source libraries with effective outcomes. However, the Metamorphic Relations (MRs) used in the work of Mao et al. \cite{cmao2024} are designed specifically for recommender systems. Therefore, while these MRs provide a good example of MRs they are not relevant to our work.
	...
	Moreover, they suggest doing more research on automating MRs generation and MRs statistical testing. This paper gave a proper introduction to the challenges with applying metamorphic testing on machine learning algorithms in general and that helped us to start working on QSVM. 
	...
	Moreover, Jia et al. \cite{mjia2020} focus on combining metamorphic relations in a compositional way to enhance bug detection. This work provides results of compositional metamorphic relations having an accuracy of 100\% in detecting bugs compared to the rate of one-dimensional (solo) metamorphic relation being 86\%. The compositional approach is applicable to our work and can be considered in future.
	
	Rehman and Srinivasan \cite{frehman2023} give an overview of using MRs with machine learning and elaborated on the challenges without much focus on testing supervised algorithms, which as a result is not linked directly to our work. Moreover, Kang's work \cite{dkang2022} focuses on joining fuzz testing and metamorphic testing with two MR definitions. While this can be considered an option in the future, the current work is initially to just apply metamorphic testing. Furthermore, Xiao et al. \cite{dciao2022} target deep learning compilers and not QSVM particularly, which is the system under test in our work. Deng et al.'s \cite{ydeng2023} work is focused on MRs for autonomous driving which is irrelevant to the test in our work. Jia et al. \cite{mjia2020} is combining MRs in a compositional manner, which inspired our work to be able to use more than one MR at once.
	...
	To the best of our knowledge, the field of testing quantum machine learning algorithms is still unexplored. This field can be traversed by starting with testing quantum support vector machines, and potentially later on a quantum neural networks algorithm, as an advanced step. Quantum machine learning algorithms face significant verification and debugging challenges due to their non-deterministic probabilistic behavior. It was shown that metamorphic testing is very effective in testing such systems as a black box and therefore this is a good research area to be addressed.
	
	
	\section{Metamorphic Relations for Quantum Support Vector Machine}
	\label{sec:Metamorphic_Relations_for_Quantum_Support_Vector_Machine}
	
	\subsection{Methodology}
	
	\subsubsection{Finding Metamorphic Relations}
	
	Our methodology for finding Metamorphic Relations (MRs) for QSVM was divided into two main phases. Since QSVM is a combination of machine learning and quantum computing, we treated the whole system as one pipeline in phase one with system-level MRs. And in phase two, we componentized the system and focused on the quantum component more with kernel-level MRs. For system-level MRs, we have two categories of MRs, which are for SVM and for QC. The initial SVM MRs are taken from the work of Dwarakanath et al. \cite{Dwarakanath2018}. Most of the initial QC MRs have been taken by the work of Paltenghi et al. \cite{Paltenghi2023} with our addition of applying statistical testing. The kernel-level Metamorphic Relations were extracted from different papers, as mentioned in section \ref{sec:kernel_mrs}.
	
	These system-level metamorphic relations all assess the output accuracy. Metamorphic relations can be classified based on the type of transformation they apply. Following the taxonomy proposed by \cite{8785455}, we identified two key patterns
	as shown in Table \ref{tab:mr_patterns}.
	
	\begin{table}[h!]
		\centering
		\caption{Metamorphic Relation Patterns}
		\label{tab:mr_patterns}
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{|l|p{5.5cm}|p{5.5cm}|}
				...
				\hline
				MRIP &
				Transforms the \textit{input} before feeding it to the SUT;
				the output is expected to remain unchanged. &
				Scaling (system-level): Multiply all inputs by $c$;
				accuracy of $f(c\mathbf{x})$ should equal $f(\mathbf{x})$.
				Symmetry (kernel-level): Swap kernel inputs;
				$K(x,y)$ should equal $K(y,x)$. \\
				\hline
				MROP &
				Defines an expected relationship on the \textit{output}
				without transforming the input data. &
				Invert Labels (system-level): Labels are flipped
				($y \to 1-y$); accuracy remains the same.
				Identity (kernel-level): $K(x,x)$ must equal $1$;
				the expected output value is directly specified. \\
				\hline
			\end{tabular}%
		}
	\end{table}
	
		To systematically find metamorphic relations, we classified the MRs found in the
	literature into four source categories. These categories serve as a guideline for
	deriving new MRs for QSVM or similar systems.
	
	\begin{enumerate}
		\item \textbf{Data Transformation (Maps to MRIP):} This is a category that utilizes directly the input patterns MRIP, where MRs are derived from invariance properties of the
		ML algorithm under changes to the input data. This category
		is inspired by the work of Dwarakanath et al. \cite{Dwarakanath2018} and Santos
		et al. \cite{ssantos2020}, and includes our system-level MR\#1--\#5 (e.g., scaling,
		rotation, permutation). New MRs can be found by identifying additional
		operations (e.g., translation, reflection) that preserve the algorithm functionality.
		
		\item \textbf{Quantum Circuit Equivalence (Maps to MRIP):} MRs derived from semantic-preserving
		modifications to the quantum circuit structure that preserve the unitary operator.
		This category is inspired by Paltenghi et al. \cite{Paltenghi2023} and Garcia
		Gil \cite{gracia2024}, and includes our system-level MR\#6--\#12 (e.g., adding a
		quantum register, injecting null-effect operations). New MRs can be found by
		identifying new changes to the circuit that will result in the same unitary. This category keeps the output invariant while it configures the input during apply equivalent changes to the circuit topology and this aligns with the MRIP pattern.
		
		\item \textbf{Mathematical Kernel Properties (Maps to MROP \& MRIP):} MRs derived from the formal properties of inner products and kernel functions. This category is inspired by
		Schuld and Killoran \cite{schuld2019} and Xie et al. \cite{xie2011}, and includes
		our kernel-level MR\#1--MR\#4 (e.g., symmetry, identity, negation). Relations like Identity ($K(x,x) = 1$) dictate an absolute expected output without transforming a prior data pair, mapping directly to Metamorphic Relations with Output Patterns (MROP). Conversely, properties like Symmetry ($K(x,y) = K(y,x)$) transform the input order, mapping to MRIP. New MRs can be found from any formally provable property of the kernel function such as positive semi-definiteness.
		
		\item \textbf{Embedding-Specific Properties (Maps to MRIP):} MRs derived from the mathematical
		structure of the chosen feature maps. This category is inspired by Havlicek et al. \cite{havlicek2019} and Mottonen et al. \cite{mottonen2004}, and includes some kernel-level MRs like MR\#6, MR\#10, MR\#14. New MRs can be found by studying the mathematical constraints of feature maps.
	...
	\label{sec:validating_metamorphic_relations}

At the system-level, the validation starts with comparing the accuracy of the original model and the mutated model. If they are not equal then only at this case we proceed with statistical testing because this discrepancy doesn't mean a bug was found because machine learning training results might vary. The statistical testing incorporates cross validation as well to make sure the results are fair. Finally a MR is considered successful in detecting a fault and killing a mutant only if the accuracies differences are proven to be significantly different.

At the kernel-level, to ensure the validity of the MRs, specifically the kernel-level MRs, we implemented a differential testing strategy \cite{mckeeman1998} known as the Golden Matrix approach as shown in Figure \ref{fig:golden_matrix_flow}. We verify the intermediate Kernel Matrix \cite{wang2021} on the data to identify which mutants are changing the kernel function drastically, and these caught mutants will be used to measure our Quantum Kernel Metamorphic Relations efficiency, as shown in Figure \ref{fig:golden_matrix_flow}. 
	...
	We defined twelve system-level metamorphic relations mathematically to assess the overall machine learning classification robustness. These relations are categorized in two groups of five system-level SVM-related MRs and seven system-level QC-related MRs. The first group consists of five system-level SVM-related MRs as shown below:

		To formalize these relations, we define the original training dataset as $tr = \{(\mathbf{x}_i, y_i)\}_{i=1}^N$, where $\mathbf{x}_i \in \mathbb{R}^d$ represents the feature vectors and $y_i \in \{0, 1\}$ represents the labels. Let $f_{pre}(\mathbf{x})$ be the classifier trained on $tr$ evaluated at a test input $\mathbf{x}$. The modified training dataset is $tr'$, the modified test input is $\mathbf{x}'$, and the classifier for modified data is $f_{pre'}(\mathbf{x}')$.
		
		\begin{enumerate}
			\item \textbf{Scaling (MR\#1):} Multiplying all feature inputs by a constant scalar $c \in \mathbb{R}^+$. The spatial relationship between data points is preserved.
			$$ tr' = \{(c\mathbf{x}_i, y_i)\}_{i=1}^N, \quad \mathbf{x}' = c\mathbf{x} $$
			$$ f_{pre'}(\mathbf{x}') = f_{pre}(\mathbf{x}) $$
			
		\item \textbf{Rotation (MR\#2):} Rotating all features uniformly by a constant angle. Mathematically, this is applied via an orthogonal rotation matrix $R$, to not stretch or shrink the data.
		$$ tr' = \{(R\mathbf{x}_i, y_i)\}_{i=1}^N, \quad \mathbf{x}' = R\mathbf{x} $$
		$$ f_{pre'}(\mathbf{x}') = f_{pre}(\mathbf{x}) $$
			
			\item \textbf{Feature Permutation (MR\#3):} Swapping the index order of features uniformly across the entire dataset using a permutation matrix $P$.
			$$ tr' = \{(P\mathbf{x}_i, y_i)\}_{i=1}^N, \quad \mathbf{x}' = P\mathbf{x} $$
			$$ f_{pre'}(\mathbf{x}') = f_{pre}(\mathbf{x}) $$
			
		\item \textbf{Inverting Labels (MR\#4):} Reversing all the classification labels in both the training data and the test data ($y \to 1 - y$). The overall testing accuracy must remain exactly equal.
		
		$$ tr' = \{(\mathbf{x}_i, 1 - y_i)\}_{i=1}^N, \quad test' = \{(\mathbf{x}_j, 1 - y_j)\}_{j=1}^M $$
		$$ f_{pre'}(\mathbf{x}) = 1 - f_{pre}(\mathbf{x}) \implies \text{Accuracy}_{test'} = \text{Accuracy}_{test} $$
			
			\item \textbf{Duplicating Data Inputs (MR\#5):} Adding identical rows to the dataset to test weighting stability. Duplicating the dataset should not alter the hyperplane.
			$$ tr' = tr \cup tr, \quad \mathbf{x}' = \mathbf{x} $$
			$$ f_{pre'}(\mathbf{x}') = f_{pre}(\mathbf{x}) $$
		\end{enumerate}
		
	In Figure \ref{fig:SVM_MRs}, we have the SVM-related system-level metamorphic relations, which consists of four sub-figures. In Figure \ref{fig:sub-1-0}, QSVM classification is shown. With the first MR\#1 of scaling inputs, it is shown in Figure \ref{fig:sub-1-1} that the classification are not affected. Similarly with MR\#2, rotating all the inputs by the same angle will not affect the classification as shown in Figure \ref{fig:sub-1-2}. Furthermore, from MR\#3 to MR\#5, they are outlined in Figure \ref{fig:sub-1-3}.
	...
	\end{figure}
		...
		Then we got the second group of seven system-level MRs that are QC-related. To formalize these relations, we define the quantum kernel $K(\mathbf{x}, \mathbf{y})$ as the squared inner product of the embedded data states. For Amplitude Embedding, the state is $\Phi(\mathbf{x}) = \sum_{i=1}^{n} x_i \psi_i$. For Angle Embedding, the state is $\Psi(\mathbf{x}) = \sum_{i=1}^{n} r_i e^{i \theta_i(x_i)} \psi_i$. The seven system-level MRs that are QC-related are as below:
		
		\begin{enumerate}
			\setcounter{enumi}{5}
			\item \textbf{Add Quantum Register (MR\#6):} Appending an unentangled ancillary register $\ket{0}_{anc}$ expands the Hilbert space but does not alter the core feature overlap, as $\langle 0 | 0 \rangle = 1$:
			\begin{itemize}
				\item \textit{Angle:} $\Psi'(\mathbf{x}) = \Psi(\mathbf{x}) \otimes \ket{0}_{anc} \implies |\langle \Psi'(\mathbf{x}) | \Psi'(\mathbf{y}) \rangle|^2 = K(\mathbf{x}, \mathbf{y})$
				\item \textit{Amplitude:} $\Phi'(\mathbf{x}) = \Phi(\mathbf{x}) \otimes \ket{0}_{anc} \implies |\langle \Phi'(\mathbf{x}) | \Phi'(\mathbf{y}) \rangle|^2 = K(\mathbf{x}, \mathbf{y})$
			\end{itemize}
			
			\item \textbf{Inject Null Effect Operations (MR\#7):} Inserting self-canceling unitary quantum gates into the state preparation circuit. Let the original feature map unitary be $U(\mathbf{x}) = U_2(\mathbf{x}) U_1(\mathbf{x})$. We inject an operation $V$ and its adjoint $V^\dagger$ (where $V V^\dagger = I$). The modified unitary resolves to the original:
			$$ U'(\mathbf{x}) = U_2(\mathbf{x}) V V^\dagger U_1(\mathbf{x}) = U_2(\mathbf{x}) I U_1(\mathbf{x}) = U(\mathbf{x}) $$
			Because the global unitary operator is strictly preserved, applying it to the initial state $\ket{0}^{\otimes n}$ yields the exact same state vectors for both embedding feature maps:
			\begin{itemize}
				\item \textit{Angle:} $\Psi'(\mathbf{x}) = U'(\mathbf{x})\ket{0}^{\otimes n} = U(\mathbf{x})\ket{0}^{\otimes n} = \Psi(\mathbf{x})$
				\item \textit{Amplitude:} $\Phi'(\mathbf{x}) = U'(\mathbf{x})\ket{0}^{\otimes n} = U(\mathbf{x})\ket{0}^{\otimes n} = \Phi(\mathbf{x})$
			\end{itemize}
			$$ \implies K'(\mathbf{x}, \mathbf{y}) = K(\mathbf{x}, \mathbf{y}) $$
			
		\item \textbf{Inject Parameters (MR\#8):} Substituting a fixed quantum gate used in state preparation with a parameterized rotation gate set to an equivalent value. For example, replacing a standard Pauli-$X$ gate with an $R_x$ rotation gate parameterized by $\theta = \pi$:
		$$ R_x(\pi) = \begin{bmatrix} \cos(\pi/2) & -i\sin(\pi/2) \\ -i\sin(\pi/2) & \cos(\pi/2) \end{bmatrix} = \begin{bmatrix} 0 & -i \\ -i & 0 \end{bmatrix} = -iX \equiv X $$
		Because the global phase $-i$ is physically unobservable, the gates are operationally equivalent. Applying this substitution preserves the final measurable state overlap:
		\begin{itemize}
			\item \textit{Angle:} $\Psi'_{R_x}(\mathbf{x}) \equiv \Psi_X(\mathbf{x}) \implies K'(\mathbf{x}, \mathbf{y}) = K(\mathbf{x}, \mathbf{y})$
			\item \textit{Amplitude:} $\Phi'_{R_x}(\mathbf{x}) \equiv \Phi_X(\mathbf{x}) \implies K'(\mathbf{x}, \mathbf{y}) = K(\mathbf{x}, \mathbf{y})$
		\end{itemize}
		
			\item \textbf{Change of Testing Device (MR\#9):} Executing the feature map on a different idealized backend simulator $\mathcal{E}$. The underlying theoretical state vectors $\Phi(\mathbf{x})$ and $\Psi(\mathbf{x})$ must remain consistent regardless of the execution environment.
			$$ K_{\mathcal{E}_1}(\mathbf{x}, \mathbf{y}) = K_{\mathcal{E}_2}(\mathbf{x}, \mathbf{y}) $$
			
		\item \textbf{Change of Optimization Level (MR\#10):} Compiling (transpiling) the state preparation circuit using different optimization levels ($L$). Higher optimization levels actively rewrite the circuit to use fewer physical gates and reduce circuit depth. However, the logical unitary matrix $U_L(\mathbf{x})$ generated by the compiled circuit must remain mathematically equivalent across all levels. Let $L1$ and $L2$ be two different optimization levels:
		$$ U_{L1}(\mathbf{x}) \equiv U_{L2}(\mathbf{x}) $$
		Because the unitary transformation is preserved, the prepared quantum states are identical, giving the exact same kernel output:
		\begin{itemize}
			\item \textit{Angle:} $\Psi_{L1}(\mathbf{x}) = \Psi_{L2}(\mathbf{x}) \implies K_{L1}(\mathbf{x}, \mathbf{y}) = K_{L2}(\mathbf{x}, \mathbf{y})$
			\item \textit{Amplitude:} $\Phi_{L1}(\mathbf{x}) = \Phi_{L2}(\mathbf{x}) \implies K_{L1}(\mathbf{x}, \mathbf{y}) = K_{L2}(\mathbf{x}, \mathbf{y})$
		\end{itemize}
			
		\item \textbf{Reverse QC Wires (MR\#11):} Reversing the physical mapping of the qubits across the register ($q \to n-1-q$). This transforms the original state preparation unitary $U$ into a mirrored version $U'$. When evaluating the quantum kernel, applying this reversal symmetrically ensures the mirrored operations and their adjoints ($U'^\dagger$) maintain the exact original state overlap. The final kernel is preserved:
		\begin{itemize}
			\item \textit{Angle:} The tensor product of independent rotations is applied in reverse wire order, $U'(\mathbf{x}) = \bigotimes_{i=n}^{1} R_y(x_i) \implies K'(\mathbf{x}, \mathbf{y}) = K(\mathbf{x}, \mathbf{y})$.
			\item \textit{Amplitude:} The sequence of parameterized rotations and entangling gates is mirrored across the reversed wires, transforming $U(\mathbf{x}) \to U'(\mathbf{x}) \implies K'(\mathbf{x}, \mathbf{y}) = K(\mathbf{x}, \mathbf{y})$.
		\end{itemize}
			
			\item \textbf{Switch Q-Kernel Inner Product Inputs (MR\#12):} The magnitude of the quantum state overlap is symmetric. 
			\begin{itemize}
				\item \textit{Angle:} $K(\mathbf{x}, \mathbf{y}) = |\langle \Psi(\mathbf{x}) | \Psi(\mathbf{y}) \rangle|^2 = |\langle \Psi(\mathbf{y}) | \Psi(\mathbf{x}) \rangle|^2 = K(\mathbf{y}, \mathbf{x})$
				\item \textit{Amplitude:} $K(\mathbf{x}, \mathbf{y}) = |\langle \Phi(\mathbf{x}) | \Phi(\mathbf{y}) \rangle|^2 = |\langle \Phi(\mathbf{y}) | \Phi(\mathbf{x}) \rangle|^2 = K(\mathbf{y}, \mathbf{x})$
			\end{itemize}
		\end{enumerate}
	
	
	In Figure \ref{fig:QC_MRs}, we display the angle embedding feature map and the metamorphic relations related to it as shown in the eight sub-figures. In Figure \ref{fig:sub-2-0}, the original circuit is shown. In Figure \ref{fig:sub-2-1}, the circuit original outcome should not be changed by adding an extra quantum register as a wire. In Figures \ref{fig:sub-2-2} and \ref{fig:sub-2-3}, adding two of the same gates in sequence or applying a general gate with an equivalent parameter respectively will result in the same functionality of the QC circuit. Then in Figure \ref{fig:sub-2-4}, it shows that changing the device to transpile an equivalent circuit does not change the process. Moreover, in Figure \ref{fig:sub-2-5}, changing the circuit optimization level as a metamorphic relation keeps the circuit the same. And when we reverse the QC wires as shown in Figure \ref{fig:sub-2-7} or when we switch the inner product inputs for the quantum kernel as shown in Figure \ref{fig:sub-2-8}, we are not changing the QSVM functionality which means that the classification outcome remains the same.
	...
	Transitioning to the quantum kernel, we identified 14 kernel-level Metamorphic Relations targeting the kernel evaluation. They are divided into three validation groups based on their origin:
	...
	\begin{itemize}
		\item \textbf{MR1 (Symmetry)}: $K(\mathbf{x},\mathbf{y}) = K(\mathbf{y},\mathbf{x})$ 
		\item \textbf{MR2 (Identity)}: $K(\mathbf{x},\mathbf{x}) = 1$
		\item \textbf{MR3 (Permutation)}: Invariant to the simultaneous shuffling of features in both input vectors \cite{xie2011}.
		\item \textbf{MR4 (Negation)}: $K(-\mathbf{x},-\mathbf{y}) \approx K(\mathbf{x},\mathbf{y})$. Invariant to sign inversion across all features for all standard embedding feature maps \cite{xie2011}.
		\item \textbf{MR5 (Feature Sensitivity)}: A metamorphic change relation ensuring every input dimension is functionally active. Mathematically, let $\mathbf{x}^{(i \to 0)}$ represent vector $\mathbf{x}$ with its $i$-th feature forced to zero. For any $x_i \neq 0$, the relation is defined as: $K(\mathbf{x}^{(i \to 0)}, \mathbf{y}) \neq K(\mathbf{x}, \mathbf{y})$ \cite{Segura2016}.
	\end{itemize}
	
	\paragraph{Group B: Quantum-Specific Relations}
	These relations adopt the concept of Quantum metamorphic relations defined in the MorphQ framework \cite{Paltenghi2023}. They target properties unique to quantum circuits and physical qubit wiring.
	
	\begin{itemize}
		\item \textbf{MR6 (Noise Stability)}: Evaluates the system's robustness against small input changes. Mathematically, if we apply a small shift $\boldsymbol{\delta}$ to the input feature $\mathbf{x}$, the quantum kernel value should not change beyond a tolerance threshold $\epsilon$:
		$$ |K(\mathbf{x} + \boldsymbol{\delta}, \mathbf{y}) - K(\mathbf{x}, \mathbf{y})| \le \epsilon $$
		We determined $\epsilon$ to be between $0.01$ and $0.05$. This range was chosen based on the natural quantum shot noise (which is roughly $1/\sqrt{\text{shots}}$). Setting $\epsilon$ in this range ensures we do not flag normal quantum randomness as a bug (false positives), while still keeping the SVM hyperplane stable.
		\item \textbf{MR7 (Structural Orthogonality)}: Verifies that orthogonal quantum states remain non-overlapping \cite{schuld2019}: $K(\mathbf{x}, \mathbf{x}_{\perp}) \approx 0$
		\item \textbf{MR8 (Dual Consistency)}: Validates that reversing the input feature order maintains the same relationship, testing the consistency of the entanglement chain \cite{Paltenghi2023}: $K(\text{rev}(\mathbf{x}), \text{rev}(\mathbf{y})) \approx K(\mathbf{x}, \mathbf{y})$
		\item \textbf{MR9 (Qubit Permutation)}: Ensures that changing the qubit mapping (wire map) does not alter the kernel output \cite{Paltenghi2023}.
		\item \textbf{MR10 (Feature Rotation)}: Invariant to cyclic shifts of the feature indices within the input vector, testing the consistency of the physical wire mapping \cite{LaRose2020}. Mathematically, let $C_k(\mathbf{x})$ be a cyclic shift operator that rotates the elements of vector $\mathbf{x}$ by $k$ positions: $K(C_k(\mathbf{x}), C_k(\mathbf{y})) \approx K(\mathbf{x}, \mathbf{y})$.
	\end{itemize}
	
	\paragraph{Group C: Feature-Embedding Specific Relations}
	These relations evaluate properties that arise from the mathematical constraints of the chosen data encoding strategy (Angle, Basis, or Amplitude) \cite{zaman2025surveyquantummachinelearning, schuld2018}. Because they exploit specific geometric structures, they uniquely fail if applied to the wrong embedding type.
	
	\begin{itemize}
		\item \textbf{MR11 (Shift Invariance)} \textit{[Target: Angle Embedding]}: $K(\mathbf{x}+c, \mathbf{y}+c) \approx K(\mathbf{x},\mathbf{y})$. Adding a constant shifts both angles equally, preserving relative distance on the Bloch sphere. This fails for \textit{Amplitude} because linear shifts alter the geometric angle between vectors after normalization, and fails for \textit{Basis} as it introduces non-binary values \cite{havlicek2019}.
		
		\item \textbf{MR12 (Periodicity)} \textit{[Target: Angle Embedding]}: $K(\mathbf{x}+2\pi, \mathbf{y}) \approx K(\mathbf{x},\mathbf{y})$. Rotation gates repeat their exact state every $2\pi$ radians. This fails for \textit{Amplitude} because adding $2\pi$ drastically changes the relative weighting of the state vector's components. Since the vector must be re-normalized, the resulting quantum state $\Phi(\mathbf{x}')$ represents a completely different point in the Hilbert space. This also fails for \textit{Basis} as $x+2\pi$ is not a valid binary $\{0, 1\}$ input \cite{Sweke2020, havlicek2019}.
		
		\item \textbf{MR13 (Bit Inversion)} \textit{[Target: Basis Embedding]}: $K(1-\mathbf{x}, 1-\mathbf{y}) \approx K(\mathbf{x},\mathbf{y})$. Inverting binary bits preserves the count of matching/mismatching bits. This fails for \textit{Angle} because non-linear entangling products (e.g., $x_1 x_2 \neq (1-x_1)(1-x_2)$) are reshuffled, and fails for \textit{Amplitude} as $1-x$ introduces cross-terms that change the geometric angle between normalized vectors \cite{Dwarakanath2018}.
		
		\item \textbf{MR14 (Linear Scaling)} \textit{[Target: Amplitude Embedding]}: $K(c\mathbf{x}, c\mathbf{y}) \approx K(\mathbf{x},\mathbf{y})$. Data is normalized prior to state preparation, so scalars factor out ($\frac{c\mathbf{x}}{\|c\mathbf{x}\|} = \frac{\mathbf{x}}{\|\mathbf{x}\|}$). This fails for \textit{Angle} because scaling rotation angles changes the relative distance on the Bloch sphere, and fails for \textit{Basis} as scaling produces invalid non-binary states \cite{mottonen2004}.
	\end{itemize}
	...
	\subsection{Research Questions}
		...
		The research questions were derived from the gaps identified in the literature review(Section \ref{sec:LiteratureReview}). The literature shows that metamorphic testing is effective for classical ML \cite{Dwarakanath2018, ssantos2020} and for quantum computing programs \cite{Paltenghi2023, gracia2024}, but no work has applied it to quantum machine learning algorithms like QSVM. This raised the question of which MRs would be effective (RQ1) and whether existing SVM MRs would transfer to the quantum setting (RQ2). Finally, the low kill rate observed in our first system-level experiment motivated investigating whether focusing on the quantum kernel alone would yield better results (RQ3).
	...
	\begin{itemize}
		...
		\item RQ3: How efficient will be metamorphic testing if applied only on the quantum kernel?
	\end{itemize}
	
	Collectively, these research questions are designed to progressively evaluate the
	applicability of metamorphic testing to QSVM at different levels of granularity.
	RQ1 and RQ2 establish a baseline by assessing whether known MRs from classical
	SVM and quantum computing literature are effective. RQ3 then narrows the
	scope from the full pipeline to the quantum kernel component, testing whether a
	more targeted approach improves defect detection. Together, they aim to demonstrate
	that metamorphic testing is a viable and systematic method for testing quantum
	machine learning algorithms, and to identify at which abstraction level it is most
	effective.
	...
	Where $y^i_{test}$ is the testing data, $M$ is the model, and $\bar{M}$ is the modified data model. There are three factors to differentiate between the modified data models. The first factor is $k$ that represents which k-fold data was used. The second factor is $j$ that represents which metamorphic relation (MR) is used, and $v$ that represents what value is used with the metamorphic relation. For example, if we pick k-fold of value 1, MR of value 1 which is multiplying all the data by a scalar of value $v$ and the picked scalar value was 5, then the model will be named as $M_{11,1,5}$. $x^i_{test}$ represents the inputs and $\mid \bar{MS} \mid$ is the number of all models. The MTS should be equal to 0 which indicates that no bugs were found in the implementation of QSVM. The p-value shows if the null hypothesis is more likely or the alternative hypothesis. In our experiment, the p-value threshold is $0.05$ and p-value has to be bigger than that to support the null hypothesis ($H_0$), which implies that there is no statistically significant difference between the accuracy of the original data and the modified data. The alternative hypothesis ($H_1$) states that a statistically significant difference exists.
	...
	To achieve sub-circuit mutation testing, we modified the code to be more expressive and explicit, and we have added a third feature map known as basis embedding to the test. Basis embedding maps binary inputs directly to the computational basis states of the qubits \cite{schuld2018}:
	...
	Moreover, we have transitioned from creating mutations manually to a dynamic approach for creating mutations, and we categorized the mutations in three classes to line up with the literature of mutations generation \cite{9844849, qMutphyFortunato}:
	\begin{itemize}
	...
	\subsection{Results}
		...
		In the beginning, we tested with the MR\#1 (data scaling) and MR\#2 (angle rotation) from system-level MRs without applying statistical testing and mutation analysis. The experiment code was in Python using Pennylane framework for QML, and Qiskit for QC. The data used was Wine Data from Table \ref{tab:Datasets}, since working with a larger dataset takes a lot of time with a local simulator. Then the setup included applying cross-validation of 16 i.e., k-fold values from 0 to 15, the MR\#1 which is scaling the data, and the scaling values were $[2,20)$ i.e., from 2 to 19. We applied the first two types of noise (Depolarizing channel noise and bit-flip noise) for values from 0.1 to 0.9. To implement noise, the device configuration must be transitioned from lightning.qubit to default.mixed, as illustrated in Figure \ref{fig:5}. For the future, to use a back-end device, the physical IBM quantum hardware can be accessed via the qiskit.remote device type (see Figure \ref{fig:5}).
	...
	As shown from Figure \ref{fig:4}, all the models accuracy in the initial testing phase with Wine Data from Table \ref{tab:Datasets}, were identical, and even by adding different noise types and values, the accuracy remained the same. These findings suggest that the QSVM operates as a deterministic rather than a stochastic system. In the subsequent phases, and then mutation operators were added to introduce bugs to QSVM program, to measure the metamorphic relations more effectively \cite{9678563}. 
	...
	Our subsequent evaluation revealed that introducing a specific number of shots to local simulators effectively injects randomness into the system, serving as an alternative to using a dedicated noisy device. In Figure \ref{fig:6} we show how the mutations are implemented in the QSVM program. We are using intermediate managers to switch between mutated classes according to the mutation number provided. For example, if we are testing mutation \#1, then the feature map used will be from the \#1 mutated class of feature map class. All mutants details can be found in Figures \ref{fig:11} through \ref{fig:18} in Appendix \ref{app:QSVMMutationsTable}.
	
	\begin{figure}[htbp]
		\includegraphics[height=13.2cm, width=\linewidth]{figures/Mutants_Classes.png}
	...
	The final results for the system-level pipeline testing are shown in Figure \ref{fig:8}. First, the mutants shown in blue are labeled as equivalent and they are the results of the validation step as mentioned in section \ref{sec:validating_metamorphic_relations}. MR\#1 killed the mutant \#1 and mutant \#26. MR\#2 killed mutant \#26, and MR\#3 killed mutant \#34. In Figure \ref{fig:9}, we can find that 10\% of the mutants were killed. 
	
	\begin{figure}[htbp]
		\includegraphics[height=20.2cm, width=\linewidth]{figures/mutant_outcome_fully_hierarchical_heatmap_sequential_labels_fixed_data.png}
		\caption{QSVM System-Level Test Results}
		\label{fig:8}
	\end{figure}
	
	\begin{figure}[H]
		\includegraphics[height=12.2cm, width=\linewidth]{figures/mutant_detection_coverage_pie_chart.png}
		\caption{System-Level Test - Killed Mutants Percentage}
		\label{fig:9}
	\end{figure}
	
	
	To address the low kill rate of system-level relations, the targeted quantum kernel experiment was conducted with 100 runs since the mutator is non-deterministic, and to provide statistical results. 
	In Figure \ref{fig:v41_confidence}, we can see that 96\% of the mutants were identified by the golden matrix as worthy mutants. There were 25 mutants generated for each feature map and only one of each did not pass the golden matrix validation to be tested. This shows that our validation method works, where we mentioned it in  section \ref{sec:validating_metamorphic_relations}.
	...
	\begin{itemize}
		\item \textbf{Basis Embedding}: Most of the mutants were killed by MR\#2 and then followed by MR\#5
		\item \textbf{Angle Embedding}: Most of the mutants were killed by MR\#1, MR\#2, MR\#4, and MR\#11 equally. MR\#3 almost killed all mutants in all runs, and majority were killed by MR\#7. MR\#12 managed to killed nearly half.
		\item \textbf{Amplitude Embedding}: MR\#1 and MR\#2 killed the majority of mutants, and half of the mutants were killed by MR\#6, then followed by MR\#4 and MR\#14 with equal percentages.
	\end{itemize}
	
		\begin{figure}[H]
		\centering
		\includegraphics[height=18.2cm, width=1.0\textwidth]{figures3/chart_v41_heatmap_omni_2_cmp_mr_only.png}
	...
	At a high level, QSVM is still more of a machine learning algorithm rather than a quantum algorithm. Subsequently, testing QSVM as a whole pipeline with different metamorphic relations showed that the metamorphic relations related to SVM were more effective than the ones related to Quantum Computing. However, the possibility stands that there could be more effective metamorphic relations that could be used and could be more effective. But, finding these metamorphic relations will need more work and research. 
	
	In the last experiment, when we tested only the quantum element in QSVM, we managed to kill almost all the mutants using 14 quantum kernel metamorphic relations with a success rate of 96\%. The kernel-level testing phase was generally more promising than the system-level testing phase.
	...
	The Gantt chart for the study plan is shown in the appendix (see Figure \ref{fig:10}). The first thing to do next is start working on Quantum Convolutional Neural Networks (QCNNs) after the upgrade. Then, Statistical testing will be applied on QCNNs along with mutation testing. 
