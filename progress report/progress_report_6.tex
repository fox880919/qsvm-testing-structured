...
\begin{document}
	...
	Section \ref{sec:preliminaries} goes through the preliminaries about QC and software testing.  Section \ref{sec:LiteratureReview} will review the literature and relatively recent research around software testing with quantum programs and machine learning algorithms. Section \ref{sec:WorkCompleted} will be about the completed work. Section \ref{sec:challenges_solutions} elaborates the challenges faced, the subsequent solutions, and the advanced experimentation. Section \ref{sec:next-plan} shows the work plan for the next period.
	...
	. Moreover, they suggest to do more research on automating MRs generation and statistical MR testing. This paper gave a proper introduction to the challenges with applying metamorphic testing on machine learning algorithms in general and that helped us to start working on QSVM. 
	...
	\section{Research \& Work Completed}
	\label{sec:WorkCompleted}
	In this section we will elaborate on the initial research questions, the work that has been completed and the findings.
	...
	In the following section of methodology, we will elucidate what were the selected approaches and why.
	
	For platform and libraries, we selected Qiskit SDK and Pennylane library. Both of them are tools that can be used with the Python programming language. First, the reason for selecting Python is because it is the most well-known programming language for AI development, and also for QC development. Moreover, Pennylane is an emerging library that facilitates working with QML algorithms and it is highly recommended for enabling integration between QML with Quantum hardware service providers \cite{bergholm2022pennylaneautomaticdifferentiationhybrid}.
	
	For metamorphic relations (MRs) to be used, we have two categories of MRs which are for SVM in general and for QC. The initial SVM MRs are inspired by the work of Dwarakanath et al. \cite{Dwarakanath2018}. The initial QC MRs have been inspired by the work of Paltenghi et al. \cite{Paltenghi2023} with our addition of applying statistical testing.
	
	In Table \ref{tab:All_MRs}, we show the selected MRs for both categories:
	
	\begin{table}[h!]
		\centering
		\caption{Metamorphic Relations for SVM and QC Models}
		\label{tab:All_MRs}
		\begin{tabular}{|l|l|}
			\hline
			\textbf{SVM MRs} & \textbf{QC MRs} \\
			\hline
			1. Scaling & 6. Add Quantum Register \\
			2. Rotation & 7. Inject Null Effect Operations \\
			3. Feature Permutation & 8. Inject Parameters \\
			4. Inverting Labels & 9. Change of testing device \\
			5. Duplicating data inputs & 10. Change of optimization level \\
			& 11. Reverse QC wires \\
			& 12. Switch Q-Kernel Inner Product Inputs \\
			\hline
		\end{tabular}
	\end{table}
	
	These metamorphic relations all assess the output accuracy. There are other patterns like Metamorphic Relation Input Pattern (MRIP) and Metamorphic Relation Output Pattern (MROP) \cite{8785455}. MROP can be considered for future to create some more metamorphic relations for this work. 
	
	In Figure \ref{fig:SVM_MRs}, we have the SVM related metamorphic relations, which consists of four sub-figures. In Figure \ref{fig:sub-1-0}, QSVM classification is shown. With the first MR\#1 of scaling inputs, it is shown in \ref{fig:sub-1-1} that the classification are not affected. Similarly with MR\#2, rotating all the inputs by the same angle will not affect the classification. Furthermore, from MR\#3 to MR\#5, the classification of QSVM did not change. In Figure \ref{fig:QC_MRs}, we display the angle embedding feature map and the metamorphic relations related to it as shown in the eight sub-figures. In Figure \ref{fig:sub-2-0}, the original circuit is shown. In Figure \ref{fig:sub-2-1}, the circuit original outcome should not be changed by adding an extra quantum register as a wire. In Figures \ref{fig:sub-2-2} and \ref{fig:sub-2-3}, adding two of the same gates in sequence or applying a general gate with an equivalent parameter respectively will result in the same functionality of the QC circuit. Then in Figure \ref{fig:sub-2-4}, it shows that changing the device to transpile an equivalent circuit does not change the process. Moreover, in Figure \ref{fig:sub-2-5}, changing the circuit optimization level as a metamorphic relation keeps the circuit the same. And when we reverse the QC wires as MR\#11 as shown in Figure \ref{fig:sub-2-7} or when we switch the inner product inputs for the quantum kernel, we are not changing the QSVM functionality which means that the classification outcome remains the same.
	
	Furthermore, we can use two MRs or more in one test from Table \ref{tab:All_MRs}, in a compositional manner \cite{mjia2020}.
	
	We have adopted statistical testing \cite{9159089} to aim for eliminating randomness in the results \cite{10.1145/1985793.1985795}, and mutation analysis to prepare mutants, buggy versions of the original code, to check if any mutants are detected. However in Section \ref{sec:findings}, the results show no randomness in implementing noise with Pennylane. 
	...
	The experiment code was in Python using Pennylane framework for QML, and Qiskit for QC. In our experiment, we adopted statistical testing, as shown in Figure \ref{fig:3}, to determine whether there were any coincidental differences between the models and to ensure there were no randomness \cite{10.1145/1985793.1985795} in the experiment. The metamorphic Testing Score is calculated as the following:
	...
	Furthermore, we adopted noise factors in the experiment to simulate real quantum computing noise, and we had  four different types of noise to be implemented in Pennylane as follows:
	\begin{itemize}
	...
	Our second experiment revealed that introducing a specific number of shots to local simulators effectively injects randomness into the system, serving as an alternative to using a dedicated noisy device in Figure \ref{fig:6}, we show how the mutations are implemented in QSVM program. We are using intermediate managers to switch between mutated classes according to the mutation number provided. For example, if we are testing mutation \#1, then the feature map used will be from the \#1 mutated class of feature map class. All the mutants details can be found in Figures ~\ref{fig:11}, \ref{fig:12},  \ref{fig:13},  \ref{fig:14},  \ref{fig:15},  \ref{fig:16},  \ref{fig:17},  and \ref{fig:18} in Appendix \ref{app:QSVMMutationsTable}.
	
	\begin{figure}[t!]
		\includegraphics[height = 15.2 cm ,width=\linewidth]{figures/Mutants_Classes.png}
		\caption{Mutations Implementation}
		\label{fig:6}
	\end{figure}
	
	\begin{figure}[t!]
		\includegraphics[height = 15.2 cm ,width=\linewidth]{figures/QSVM_flowchart.png}
		\caption{Flowchart of testing QSVM}
		\label{fig:7}
	\end{figure}
	
	The final results for the second experiment are shown in Figure \ref{fig:8}. MR\#1 killed the mutant \#1 and mutant \#26. MR\#2 killed mutant MR\#26, and MR\#3 killed mutant \#34. In Figure \ref{fig:9}, we can find that 10\% of the mutants were killed.
	
	\begin{figure}[t!]
		\includegraphics[height = 18.2 cm ,width=\linewidth]{figures/mutant_outcome_fully_hierarchical_heatmap_sequential_labels_fixed_data.png}
		\caption{QSVM Testing Results}
		\label{fig:8}
	\end{figure}
	
	\begin{figure}[t!]
		\includegraphics[height = 15.2 cm ,width=\linewidth]{figures/mutant_detection_coverage_pie_chart.png}
		\caption{Killed Mutants Percentage}
		\label{fig:9}
	\end{figure}
	
	When we tested in the first experiment with the scaling MR for SVM and the angle rotation MR, the metamorphic testing score was equal to 0, from Equation \ref{eq:MTS}. This shows that there is no difference between all the models regardless of the modifications, and this means there is not any stochastic factor when using QSVM in Qiskit and Pennylane. And in the second experiment, mutation operators were added to add bugs to QSVM program, to measure the metamorphic relations more effectively \cite{9678563}. 
	
	For the research questions (RQs) in this section, the answers are still under progress but here are our findings. for RQ1, MR\#1 was the most efficient because it killed two mutants. Then, we have MR\#2 and MR\#3 as the second most effective since both of them managed to kill one mutant. For RQ2, it seems we need to find more effective metamorphic relations. Also, another possible way to modify the test to apply different metamorphic relations for each component of QSVM like feature map independently. For RQ3, even though it was concluded the QSVM is not stochastic, we could still benefit from statistical testing to kill mutants. We might need to connect to a back-end device to work with real noisy devices to analyze if the MRs can cope up with noise if desired. Moreover, the mutants used were created manually, which alludes to them not being exhaustive of all different variations. The experiment can be repeated with an automation tool to create mutants or can be repeated on more different mutants created manually.

We would like to mention that more metamorphic relations can be used in testing QSVM to improve the test results presented in this Section \ref{sec:WorkCompleted}. However, finding metamorphic relations takes a lot of time, and it took a lot of time to accumulate the necessary knowledge around software testing, machine learning, and quantum computer for this report. However, in future we can work on improving the results in this report.


	\subsection{Transitioning To The Third Experiment:} 

	While the initial experiments provided a baseline, they highlighted significant limitations: manual mutation is not exhaustive, and testing the whole system might not be very effective to kill mutants created in different components of the system. These challenges necessitated the development of a more specified focus on the component that we need to test which is detailed in the following section.
	

	\section{Challenges, Solutions, and Third Experiment}
	\label{sec:challenges_solutions}
	
	
	
	\subsection{Challenges in the first and second experiments}
	One of the major challenges we faced in our experiments is that Qiskit related libraries versions need to be manually adjusted to be compatible with each other. Another challenge is that Pennylane abstracts the code and does not allow for a lot of customization as in Qiskit. Moreover, Pennylane's latest version as of this report is below 1.0 which shows that it is still in an experimental phase. Finally, training QSVM models takes a lot of time especially if we add noise. And it also depends on the number of inputs, the data dimension and how many k-folds are used, the training time for all models can take from a couple of hours to a couple of days.	The last challenge is to have an automated tool to create mutants, since the mutants used in our second experiment were manually modified.
	...
	\subsection{Solution: The third Experiment}
	Based on the previous result and the identified challenges, we conducted a third experiment to focus only on the quantum kernel, and also we addressed the limitations of manual mutant generation. Instead of testing QSVM as a comprehensive pipeline, we started testing the quantum component, since this was one of our main focus in this work. We noticed that the feature maps are used in an abstract way in Pennylane which limited our abilities to create mutants in the quantum circuits only. We are going to explore a new research question as below:
	
	\begin{itemize}
		\item RQ4: How efficient will be metamorphic testing if applied only on the quantum kernel?
	\end{itemize}
	
	 To answer this question, we have modified the code to be more expressive and explicit, and we have added a third feature map known as basis embedding to the test. Basis encoding maps binary inputs directly to the computational basis states of the qubits \cite{schuld2018}.
	$$ \mathbf{x} = \{x_1, x_2, \dots, x_N\} \in \{0,1\}^N \rightarrow |x_1 x_2 \dots x_N\rangle $$
	
	\subsubsection{QSVM Code (Healthy Baseline)}
	For the embeddings, we used explicit logic for the three feature maps. The code is outlined in Listing \ref{lst:qsvmHealthyCode}.
	...
	\subsubsection{Mutations Generation Techniques}
	
	We have transitioned from creating mutations manually to preparing a dynamic approach for creating mutations, and we categorized the mutations in three classes to line up with the literature of mutations generation \cite{9844849, qMutphyFortunato}:
	...
	\end{itemize}
	 ...
	 Which is applied and shown in Listing \ref{lst:mutationClass}.
	...
	\subsection{14 Quantum Kernel Metamorphic Relations}
	
	We followed the multi-tier approach in \cite{paltenghi2024surveytestinganalysisquantum}To align with the latest surveys. We have identified 14 metamorphic relations used for quantum kernel and classified them into three groups:
	
	\begin{itemize}
		\item General
		\item Quantum Related
		\item Feature Embedding Related
	\end{itemize}
	
	This ensures that we are having a comprehensive testing suite from kernel level \cite{zaman2025surveyquantummachinelearning}.
	...
	\begin{itemize}
		...
		\item \textbf{Amplitude Embedding Specific (MR11)} validates the linear scaling requirements of normalized states \cite{mottonen2004}: ($K(x, x_{\perp}) \approx 0$)
	...
	The experiment was conducted with 100 runs since the mutator is non-deterministic, and to provide statistical results.
	
	\subsubsection{Efficiency Analysis}
	
	In Figure \ref{fig:v41_confidence}, we can see that 96\% of the mutants were killed in general in amplitude embedding, angle embedding and basis embedding.
	...
	For RQ4, the answer is that by testing the quantum kernel, we managed kill 96\% of the mutants created that showed drastic changes in the quantum kernel functionality, in all feature maps.
	...
	In the last experiment (Section \ref{sec:challenges_solutions}), when we tested only the quantum element in QSVM, and we managed to kill almost all the mutants using 14 quantum kernel metamorphic relations.
	
	\section{Work Plan for Next Period}
	\label{sec:next-plan}
	
	One of the plans is to use Qiskit directly for QML e.g., QSVM, or another tool besides Pennylane. We found another tool named Qiskit Machine Learning and the latest version is 0.9.0 as of Dec 2025. We might need to re-implement some parts of the code and run these experiments again.  Also, we can move from QSVM to a convolutional neural networks algorithm to use a stochastic system instead of QSVM. Moreover, for mutation testing, we can work on creating an automated tool to add modifications to the program to intentionally introduce some bugs in the system under test (SUT). \cite{qMutphyFortunato}. Besides that, we can research more effective metamorphic relations to detect more mutants and kill them.  
