...
\begin{document}
	...
	Our initial endeavor focused on finding System-Level Metamorphic Relations, which we subsequently advanced to the kernel level because the results from the System-Level Metamorphic Relations were not promising. For system-level MRs to be used, we have two categories of MRs, which are for SVM in general and for QC. The initial SVM MRs are taken from the work of Dwarakanath et al. \cite{Dwarakanath2018}. Most of the initial QC MRs have been taken by the work of Paltenghi et al. \cite{Paltenghi2023} with our addition of applying statistical testing. The kernel-level Metamorphic Relations were inspired by different papers, as mentioned in section \ref{sec:sys_mrs}.
	
	These system-level metamorphic relations all assess the output accuracy. In general,
	metamorphic relations can be classified based on the type of transformation they apply.
	Following the taxonomy proposed by \cite{8785455}, we identify two key patterns
	...
	\begin{enumerate}
		\item \textbf{Data Transformation:} MRs derived from invariance properties of the
		...
		\item \textbf{Quantum Circuit Equivalence:} MRs derived from equivalent
		modifications to the quantum circuit structure that preserve the unitary operator.
		This category is inspired by Paltenghi et al. \cite{Paltenghi2023} and Garc\'ia
		Gil \cite{gracia2024}, and includes our system-level MR\#6--\#11 (e.g., adding a
		quantum register, injecting null-effect operations). New MRs can be found by
		identifying new changes to the circuit that will result in the same unitary.
		
		\item \textbf{Mathematical Kernel Properties:} MRs derived from the formal
		properties of inner products and kernel functions. This category is inspired by
		Schuld and Killoran \cite{schuld2019} and Xie et al. \cite{xie2011}, and includes
		our kernel-level MR1--MR3 and MR12 (e.g., symmetry, identity, negation). New MRs
		can be found from any formally provable property of the kernel function such as
		positive semi-definiteness.
		
		\item \textbf{Encoding-Specific Properties:} MRs derived from the mathematical
		structure of the chosen feature maps. This category is inspired by
		Havl\'i\v{c}ek et al. \cite{havlicek2019} and M\"ott\"onen et al.
		\cite{mottonen2004}, and includes our kernel-level MR4--MR11 and MR13--MR14 (e.g.,
		angle periodicity, amplitude scaling invariance, bit inversion). New MRs can be
		found by studying the mathematical constraints of feature maps.
	\end{enumerate}
...
To ensure the validity of the MRs, specifically the kernel-level MRs, we implemented a differential testing strategy \cite{mckeeman1998} known as the Golden Matrix approach as shown in Figure \ref{fig:golden_matrix_flow}. We verify the intermediate Kernel Matrix \cite{wang2021} on the data to identify which mutants are changing the kernel function drastically, and these caught mutants will be used to measure our Quantum Kernel Metamorphic Relations efficiency, as shown in Figure \ref{fig:golden_matrix_flow}. 	In system-level MRs, the workflow incorporated a comparison between the original model accuracy and the mutated model accuracy, and when the accuracy differs, then we proceeded with statistical testing, Figure \ref{fig:7}. This can be seen as a validation for both statistical testing and metamorphic relations at the same time rather than just validating metamorphic relations per se. Perhaps, we should have applied metamorphic relations without statistical testing to give them more proper validation.
	...
	\end{figure}
		...
		\paragraph{Feature Embedding \& Execution Metamorphic Relations}
		To formalize these relations, we define the quantum kernel $K(\mathbf{x}, \mathbf{y})$ as the squared inner product of the embedded data states. For Amplitude Embedding, the state is $\Phi(\mathbf{x}) = \sum_{i=1}^{n} x_i \psi_i$. For Angle Embedding, the state is $\Psi(\mathbf{x}) = \sum_{i=1}^{n} r_i e^{i \theta_i(x_i)} \psi_i$. 
	...
	\begin{itemize}
		\item \textbf{MR4 (Noise Stability)}: Evaluates robustness against small input perturbations; small $\epsilon$ shifts in $x$ should not cause divergent kernel values
		\item \textbf{MR5 (Structural Orthogonality)}: Verifies that orthogonal quantum states remain non-overlapping \cite{schuld2019}: $K(x, x_{\perp}) \approx 0$
		\item \textbf{MR6 (Dual Consistency)}: Validates that reversing the input feature order $K(rev(x), rev(y))$ maintains the same relationship as $K(x, y)$, testing the consistency of the entanglement chain \cite{Paltenghi2023}
		\item \textbf{MR7 (Qubit Permutation)}: Ensures that changing the qubit mapping (wire map) does not alter the kernel output \cite{Paltenghi2023}
	\end{itemize}
	
	
	\paragraph{Group C: Feature-Embedding Specific Relations}
	These relations evaluate properties that arise from the mathematical structure of the chosen data encoding strategy (Angle, Basis, or Amplitude) \cite{zaman2025surveyquantummachinelearning, schuld2018}.
	
	\begin{itemize}
		\item \textbf{MR8 (Shift Invariance)} \textit{[Target: Angle Embedding]}: $K(x+c, y+c) \approx K(x,y)$. Adding a constant shifts both angles equally, preserving relative distance on the Bloch sphere. This fails for \textit{Amplitude} because linear shifts alter the geometric angle between vectors after normalization, and fails for \textit{Basis} as it introduces non-binary values. \cite{havlicek2019}
		
		\item \textbf{MR9 (Periodicity)} \textit{[Target: Angle Embedding]}: $K(x+2\pi, y) \approx K(x,y)$. Rotation gates repeat their exact state every $2\pi$ radians. This fails for \textit{Amplitude} because adding $2\pi$ drastically changes the relative weighting of the state vector's components. Since the vector must be re-normalized, the resulting quantum state $\Phi(x')$ represents a completely different point in the Hilbert space. This also fails for \textit{Basis} as $x+2\pi$ is not a valid binary $\{0, 1\}$ input. \cite{Sweke2020, havlicek2019}
		
		\item \textbf{MR10 (Bit Inversion)} \textit{[Target: Basis Embedding]}: $K(1-x, 1-y) \approx K(x,y)$. Inverting binary bits preserves the count of matching/mismatching bits. This fails for \textit{Angle} because non-linear entangling products (e.g., $x_1 x_2 \neq (1-x_1)(1-x_2)$) are reshuffled, and fails for \textit{Amplitude} as $1-x$ introduces cross-terms that change the geometric angle between normalized vectors. \cite{Dwarakanath2018}
		
		\item \textbf{MR11 (Linear Scaling)} \textit{[Target: Amplitude Embedding]}: $K(cx, cy) \approx K(x,y)$. Data is normalized prior to state preparation, so scalars factor out ($\frac{c\mathbf{x}}{\|c\mathbf{x}\|} = \frac{\mathbf{x}}{\|\mathbf{x}\|}$). This fails for \textit{Angle} because scaling rotation angles changes the relative distance on the Bloch sphere, and fails for \textit{Basis} as scaling produces invalid non-binary states. \cite{mottonen2004}
		
		\item \textbf{MR12 (Negation)} \textit{[Common]}: $K(-x,-y) \approx K(x,y)$. Invariant to sign inversion across all features for all standard embedding feature maps. \cite{xie2011}
		
		\item \textbf{MR13 (Feature Rotation)} \textit{[Common]}: Invariant to cyclic shifts of the feature indices within the input vector, testing the consistency of the wire mapping. \cite{LaRose2020}
		
		\item \textbf{MR14 (Feature Sensitivity)} \textit{[Common]}: A metamorphic change relation where zeroing out a single feature must alter the resulting kernel value, ensuring every input dimension is functionally active. \cite{Segura2016}
	\end{itemize}
	...
	\subsection{Research Questions}
		...
		There are some research questions that inspired this work which are shown below. The research questions were derived from the gaps identified in the literature review(Section \ref{sec:LiteratureReview}). The literature shows that metamorphic testing is effective for classical ML \cite{Dwarakanath2018, ssantos2020} and for quantum computing programs \cite{Paltenghi2023, gracia2024}, but no work has applied it to quantum machine learning algorithms like QSVM. This raised the question of which MRs would be effective (RQ1) and whether existing SVM MRs would transfer to the quantum setting (RQ2). Additionally, the probabilistic nature of quantum computing raised the question of whether statistical testing is necessary (RQ3). Finally, the low kill rate observed in our initial system-level experiments motivated investigating whether focusing on the quantum kernel alone would yield better results (RQ4).
	...
	SVM and quantum computing literature are effective. RQ3 examines whether the
	inherent nature of QSVM necessitates statistical validation. RQ4 then narrows the
	...
	The final results for the system-level pipeline testing are shown in Figure \ref{fig:8}. MR\#1 killed the mutant \#1 and mutant \#26. MR\#2 killed mutant \#26, and MR\#3 killed mutant \#34. In Figure \ref{fig:9}, we can find that 10\% of the mutants were killed.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\textwidth]{figures2/chart_v41_heatmap_omni_2_ordered.png}
	...
	In the last experiment, when we tested only the quantum element in QSVM, and we managed to kill almost all the mutants using 14 quantum kernel metamorphic relations with a success rate of 96\%. The kernel-level testing phase was generally more promising than the system-level testing phase.
