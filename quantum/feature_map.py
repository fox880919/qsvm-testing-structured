...
def _progress_print(msg: str):
    """Print progress: use \\r for TTY (overwrite line), \\n for pipe (web capture)."""
...
class FeatureMap:
    ...
    def shot_based_kernel1(x1, x2, n_qubits):
        ...
        if DefaultParameters.testMutation != 0:
            ...
            if MyParameters.checkShiftingInvariance == True:
                
                ditance = 5
                
                x1_shifted = x1 + ditance

                x2_shifted = x2 + ditance

                tempProbabilities = quantum_amplitude_embedding_qnode(x1_shifted, x2_shifted, n_qubits)   

                FeatureMap.checkShiftingInvariance(probabilities[0], tempProbabilities[0], True)
    ...
    def shot_based_kernel2(x1, x2, n_qubits):
        ...
        if DefaultParameters.testMutation != 0:
            ...
            if MyParameters.checkShiftingInvariance == True:
                ...
                tempProbabilities = quantum_angle_embedding_qnode(x1_shifted, x2_shifted, n_qubits) 
    ...
    def changeOptimizationLevel(qnode):
        ...
        if MyParameters.allOptimizationLevels[MyParameters.selectedOptimizationLevel] == 1:
            ...
            # print(f'feature_map, changeOptimizationLevel = 1')
            # Optimization Level 1: Basic Simplification
            # Merge adjacent single-qubit rotations (highly effective for AngleEmbedding)
            transformed_qnode = qml.transforms.merge_rotations(qnode)
            level_name = "Level 1 (Merge Rotations)"
        
        elif MyParameters.allOptimizationLevels[MyParameters.selectedOptimizationLevel] == 2:

            # print(f'feature_map, changeOptimizationLevel = 2')

            # Optimization Level 2: Aggressive Simplification/Decomposition
            # First, merge rotations, then simplify Hadamard-like identity gates
            transformed_qnode = qml.transforms.commute_controlled(qml.transforms.merge_rotations(qnode))
            level_name = "Level 2 (Merge + Commute Controlled)"
    ...
    def compute_kernel_matrix(self, X1, X2, n_qubits, featureMap):
        ...
        print(colored(f'_m0, shot_based_kernel: {shot_based_kernel}', 'yellow'))
    ...
    def checkShiftingInvariance(probability1, probability2, shouldBeEqual):
