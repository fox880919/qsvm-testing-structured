...
class FeatureMap:
    ...
    def quantum_amplitude_embedding(x1, x2, n_qubits):
        ...
        FeatureMap.inputsNumber = FeatureMap.inputsNumber + 1
          ...
          #mr#11
        if MyParameters.reverseWires == False:
        ...
        #     # print(colored('feature_map, quantum_amplitude_embedding - parameters.addQuantumRegister == True', 'magenta'))
        #     # --- Add an operation on the extra wire (optional, but shows usage) ---
        #     qml.PauliX(wires=n_qubits) # Apply an X gate to the extra wire (wire n_qubits)
    ...
    def quantum_angle_embedding(x1, x2, n_qubits):
        ...
        # Return the probabilities, where the probability of the all-zero state
        # |00...0⟩ gives the kernel value.
    ...
    def compute_kernel_matrix(self, X1, X2, n_qubits, featureMap):
        ...
        print(colored(f'_m4, shot_based_kernel: {shot_based_kernel}', 'yellow'))

        print(f'featureMapType = {featureMap}')
        n_samples1 = len(X1)
        n_samples2 = len(X2)
