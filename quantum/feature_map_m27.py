...
class FeatureMap:
    ...
    def quantum_amplitude_embedding(x1, x2, n_qubits):
        ...
        if MyParameters.reverseWires == True:
            ...
            # for i in range(n_qubits): qml.RX(np.pi / 2, wires=i)         

            # qml.AmplitudeEmbedding(features=x1, wires=range(n_qubits), normalize=True)
            
            qml.adjoint(qml.AmplitudeEmbedding(features=x1, wires=range(n_qubits), normalize=True))
    ...
    def quantum_angle_embedding(x1, x2, n_qubits):
        ...
        if MyParameters.reverseWires == True:
            ...
            #mutation #27 start
            qml.adjoint(qml.AngleEmbedding(features=x2,wires=list(reversed(range(n_qubits-1))), rotation='Y'))
            #mutation #27 end
            ...
            #mutation #27 start
            qml.AngleEmbedding(features=x1, wires=range(n_qubits-1), rotation='Y')
            #mutation #27 end
            
        # qml.AngleEmbedding(x1, wires=range(n_qubits), rotation='Y')

        # Apply the inverse of the second feature vector's embedding
        qml.adjoint(qml.AngleEmbedding(features=x1, wires=range(n_qubits), rotation='Y'))
