...
class FeatureMap:
    ...
    def quantum_amplitude_embedding(x1, x2, n_qubits):
        ...
        if MyParameters.reverseWires == True:
            ...
            #mutation #4 start
            qml.adjoint(qml.AmplitudeEmbedding(features=x2, wires=list(reversed(range(n_qubits-1))), normalize=True))
            #mutation #4 end
            ...
            #mutation #4 start
            qml.adjoint(qml.AmplitudeEmbedding(features=x2, wires=range(n_qubits-1), normalize=True))
            #mutation #4 end
    ...
    def quantum_angle_embedding(x1, x2, n_qubits):
        ...
        if MyParameters.reverseWires == True:
            ...
            qml.adjoint(qml.AngleEmbedding(features=x2, wires=range(n_qubits-1), rotation='Y'))
