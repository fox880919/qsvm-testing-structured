    ...
    def quantum_amplitude_embedding(x1, x2, n_qubits):
     ...
     #mr#11
        if MyParameters.reverseWires == True:
            ...
            #mutation #5 start
            qml.AmplitudeEmbedding(features=x1, wires=list(reversed(range(n_qubits))), normalize=False)
            #mutation #5 end
            ...
            #mutation #5 start
            qml.AmplitudeEmbedding(features=x1, wires=range(n_qubits), normalize=False)
            #mutation #5 end
